...
public class GridNav : MonoBehaviour
{
    ...
    public PathfindingNode[,] creategrid()
    {
        gridx = Mathf.RoundToInt(worldsize.x / nodediameter);
        gridy = Mathf.RoundToInt(worldsize.y / nodediameter);
        PathfindingNode[,] grid = new PathfindingNode[gridx, gridy];
        ...
        for(int x = 0;x < gridx; x++)
        {
           for(int y = 0; y < gridy; y++)
            {
                grid[x, y].neighbours = neighbourFind(grid[x,y],grid);
            }
        }
        return grid;
        
    }
    public List<PathfindingNode> neighbourFind(PathfindingNode findNof, PathfindingNode[,]grid)
    {
    ...
    public List<PathfindingNode> squaremaker(PathfindingNode C, PathfindingNode[,]grid)
    {
